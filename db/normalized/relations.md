...
```mermaid
    erDiagram
        ...
        applicant {
            ...
            text path_to_profile_avatar "NOT NULL, default '' "
        ...
        cv {
            ...
            text path_to_cv_avatar  "NOT NULL, default '' "
            timestamptz created_at "NOT NULL"
            timestamptz updated_at "NOT NULL"
        }
        ...
        vacancy {
            ...
            text path_to_company_avatar "NOT NULL, default '' "
            timestamptz created_at "NOT NULL"
            timestamptz updated_at "NOT NULL"
        }
...
База данных находится во второй нормальной форме, так как находится в первой нормальной форме и каждый неключевой атрибут неприводимо зависит от первичного ключа (нельзя выявить зависимость неключевого поля от другого неключевого поля).
...
База данных находится в нормальной форме Бойса-Кодда, так как находится в третьей нормальной форме и каждая нетривиальная и неприводимая слева функциональная зависимость обладает потенциальным ключом в качестве детерминанта (достигается за счет высокой независимости всех атрибутов друг от друга).
...
- **path_to_profile_avatar** - text, NOT NULL, default ('') аватарка для профиля работника
...
- **created_at** - timestamptz, NOT NULL, дата создания аккаунта
...
- **company_description** - text, NOT NULL, CHECK (length(company_description)<150), описании компании (той части за которую ответственен этот работодатель)
- **company_website** - text, NOT NULL, CHECK (length(company_website)<50), ссылка на сайт филиала компании за которую ответственен этот работодатель
- **path_to_profile_avatar** - text, NOT NULL, default ('') аватарка для профиля работодателя
...
- **id** - int, PRIMARY KEY
- **company_name** - text, NOT NULL, UNIQUE, CHECK (length(company_name)<50), название компании
- **created_at** - timestamptz, NOT NULL, дата появления компании в нашей бд
- **updated_at** - timestamptz, NOT NULL, дата последнего обновления названия компании
...
Связь: многие произведения работодателей ко многим портфолио реализуемая с помощью промежуточной таблицы applicant_creation_to_portfolio

Relation **portfolio to applicant_creation_to_portfolio**:\
{id} -> portfolio_id\
Связь: Одно портфолио может быть указано во многих строчках applicant_creation_to_portfolio\
(в одном портфолио много работ)

Relation **applicant_creation to applicant_creation_to_portfolio**:\
{id} -> applicant_creation_id\
Связь: Одно произведение работника может быть указано во многих строчках applicant_creation_to_portfolio\
(одна работа может относится ко многим портфолио)
...
Связь: многие портфолио ко многим резюме реализуемая с помощью промежуточной таблицы cv_to_portfolio

Relation **portfolio to cv_to_portfolio**:\
{id} -> portfolio_id\
Связь: Одно портфолио может быть указано во многих строчках cv_to_portfolio\
(одно портфолио можно прикрепить к нескольким резюме)

Relation **cv to cv_to_portfolio**:\
{id} -> cv_id\
Связь: Одно резюме может быть указано во многих строчках cv_to_portfolio\
(к одному резюме можно прикрепить несколько портфолио)
...
- **job_search_status_id** - int, FK, NOT NULL, DEFAULT 1, id статуса поиска работы по этому резюме
...
- **path_to_cv_avatar** - text, NOT NULL, default ('') аватарка для резюме работника
...
Связь: Один статус поиска работы может быть указан во многих резюме\
...
Связь: многие теги произведений ко многим резюме реализуемая с помощью промежуточной таблицы cv_to_creation_tag
...
(Один тег может быть отнесен ко многим произведениям)
...
- **created_at** - timestamptz, NOT NULL, дата добавления тега произведения к вакансиям
- **updated_at** - timestamptz, NOT NULL, дата последнего обновления тега произведения в вакансии

Связь: многие теги произведений ко многим вакансиям реализуемая с помощью промежуточной таблицы vacancy_to_creation_tag
...
{id} -> creation_tag_id\
Связь: Один тег произведения может быть указан во многих строчках vacancy_to_creation_tag\
...
- **salary** - int, NOT NULL заработная плата предлагаемая сотруднику
...
Связь: многие работники ко многим вакансиям реализуемая с помощью промежуточной таблицы vacancy_subscriber
...
- **id** - bigint, PRIMARY KEY
- **applicant_id** - bigint, FK, NOT NULL, id работника выложившего произведение
- **applicant_creation_name** - text, NOT NULL, UNIQUE, CHECK (length(applicant_creation_name)<50), название произведения которое даст работник при загрузке произведения на наш сайт
...
Связь: многие работники ко многим произведениям работников (их рейтингу) реализуемая с помощью промежуточной таблицы applicant_rate_to_applicant_creation
...
## Таблица employer_rate_to_applicant_creation

- **rate** - int, рейтинг выставленный пользователем
- **employer_id** - bigint, FK, NOT NULL, id работодателя поставившего рейтинг работе
- **applicant_creation_id** - bigint, FK, NOT NULL, id произведения которой работодатель поставил рейтинг
- **created_at** - timestamptz, NOT NULL, дата когда пользователь поставил рейтинг
- **updated_at** - timestamptz, NOT NULL, дата когда пользователь последний раз обновил рейтинг

Связь: многие работодатели ко многим произведениям работников (их рейтингу) реализуемая с помощью промежуточной таблицы employer_rate_to_applicant_creation

Relation **employer to employer_rate_to_applicant_creation**:\
{id} -> employer_id\
Связь: Один работодатель может быть указан во многих строчках employer_rate_to_applicant_creation\
(Один работодатель может поставить рейтинг разным произведениям)

Relation **applicant_creation to employer_rate_to_applicant_creation**:\
{id} -> applicant_creation_id\
Связь: Одно произведение может быть указано во многих строчках employer_rate_to_applicant_creation\
(У одного произведения может быть рейтинг от многих работодателей)
